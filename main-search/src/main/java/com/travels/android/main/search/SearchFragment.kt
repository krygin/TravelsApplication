...
class SearchFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<Button>(R.id.create_new_journey_button).apply {
            setOnClickListener {
                startActivityForResult(navigationController()?.createJourneyActivityIntent(context), 1)
            }

        }
        view.findViewById<FloatingActionButton>(R.id.filters_button).apply {
            setOnClickListener {
                startActivityForResult(navigationController()?.journeysFilterActivity(context), 1)
            }
